# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 17:52:06 2019
...
class DynamicEmbeddingVariation(Layer):
    ...
    def __init__(self, input_dim, output_dim, kl_loss_weight, mask_zero=False, input_length=None, embeddings_initializer=initializers.normal(stddev=prior_sigma), **kwargs):
        ...
        super(DynamicEmbeddingVariation, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        def kern(m,s):
            return m+s*tf.random.normal(m.shape)
        #if K.dtype(inputs) != 'int32':
        #    inputs = K.cast(inputs, 'int32')
        #out = K.gather(self.embeddings, inputs)
        kernel_sigma = tf.math.softplus(self.kernel_rho)
        U=kern(self.kernel_mu,kernel_sigma)
        Mu=U/(1+kernel_sigma**2)
        Sig=1/(kernel_sigma**(-2)+1)
        kernel = kern(Mu,Sig)

        bias_sigma = tf.math.softplus(self.bias_rho)
        bias = self.bias_mu + bias_sigma * tf.random.normal(self.bias_mu.shape)
                
        self.add_loss(self.kl_loss(kernel, Mu, Sig) + 
    ...
    def get_config(self):
        ...
        base_config = super(DynamicEmbeddingVariation, self).get_config()
