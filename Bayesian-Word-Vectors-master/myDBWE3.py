# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 19:07:39 2019
...
import os
import zipfile
from Embedding_init_normal import *
...
target = EmbeddingNorm(vocab_size, vector_dim, input_length=1, name='embedding_target')(input_target)
target = Reshape((vector_dim, 1))(target)
context = EmbeddingNorm(vocab_size, vector_dim, input_length=1, name='embedding_context')(input_context)
...
for cnt in tqdm.tqdm(range(10)):
    ...
    model.fit([arr_1, arr_2], arr_3,batch_size=Batch_size,epochs=1)
    ...
    for i in range(len(Ut[0])):
        sig_u=np.std(Ut[0][i])
        Ut[0][i]=np.random.normal(Ut[0][i]/(1+sig_u**2),1/(sig_u**(-2)+1))
        
    for i in range(len(Vt[0])):
        sig_v=np.std(Vt[0][i])
        Vt[0][i]=np.random.normal(Vt[0][i]/(1+sig_v**2),1/(sig_v**(-2)+1))
