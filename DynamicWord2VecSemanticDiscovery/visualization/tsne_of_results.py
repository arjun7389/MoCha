...
Dir='D:/L3S/Mocha/DynamicWord2VecSematic-master/DynamicWord2VecSemantic-master/'
wordlist = []
fid = open(Dir+'data/wordIDHash.csv','r')
words=fid.readlines()
for line in words:
...
emb_all = sio.loadmat(Dir+'embeddings/embeddings_25.mat')

#%%

nn = 50
nc = 5
emb = emb_all['U']#_%d' % times.index(199)]
...
for year in times:
    ...
    emb = emb_all['U']#_%d' % times.index(year)]
...
plt.show()
fig.savefig(Dir+'test2png.png', dpi=100)
sio.savemat(Dir+'tsne_output/%s_tsne.mat'%word,{'emb':Z})
pickle.dump({'words':list_of_words,'isword':isword},open(Dir+'tsne_output/%s_tsne_wordlist.pkl'%word,'wb'))
...
import matplotlib.pyplot as plt
import pickle
Z = sio.loadmat(Dir+'tsne_output/%s_tsne.mat'%word)['emb']
data = pickle.load(open(Dir+'tsne_output/%s_tsne_wordlist.pkl'%word,'rb'))
